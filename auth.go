...
func requestToken(c config) config {
	...
	input := tweetgo.OAuthRequestTokenInput{
		OAuthCallback: tweetgo.String("http://localhost:3000/oauth_response"),
	}

	output, err := tc.OAuthRequestTokenPost(input)
	if err != nil {
		panic(err)
	}
	...
	accessTokenOutput, err := tc.OAuthAccessTokenPost(accessTokenInput)
	if err != nil {
		panic(err)
	}
